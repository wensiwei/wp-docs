# 日志设置与问题定位（wparse/wpgen/wpkit/wprescue）
...
### wparse / wprescue / wpkit（conf/wparse.toml）
...
带模块级别（在运行前会拼接入 `level` 串）：
```toml
[logging]
level = "info"
module_levels = ["sink=debug", "connector=debug", "stat=info"]
output = "both"
file_path = "./data/logs"
```
...
- 控制台输出：将 `output` 设为 `Console` 或 `Both`；`wpkit conf check` 可加 `--console`
- 统计到控制台（便于观察吞吐变化）：
...
2) “我设置了模块级别但不生效（wparse/wprescue/wpkit）”
- 当前仅解析 `level` 串；请将 `ctrl=info,source=debug,...` 直接写进 `level`
- wpgen 的 `[logging].module_levels` 会在启动时拼入 `level` 串，可正常生效

3) “连接器/扩展的 info 日志看不到”
- 部分扩展使用标准 `log::info!`（无 target），受 root 级别控制；将 root 调到 info：
  - wparse：`level = "info,ctrl=info,launch=info,..."`
  - wpgen：`level = "info"` 并在 `module_levels` 中细分其它域
